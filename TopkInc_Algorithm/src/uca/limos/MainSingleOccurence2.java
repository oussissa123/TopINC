...
//import uca.limos.api.topk.SetTopKAlgorithmV9;

public class MainSingleOccurence2 {
	
public static int step = 1;
public static String path = DatasetConfig.path;//"/home/ousissa/eclipse-workspace/TOP_SET_RANK_JOIN/";
...
static int e0 = 1073741822;
static int e2 = 536870912;
static int e3 = -1;
...
private static void preprocess_runtime_fig() {
	try {
...
//	System.out.println(DatasetConfig.do_preprocess);
	...
	if (DatasetConfig.fixedk) {
		...
		int X [] = {100000, 200000, 400000, 600000};
		int searchSpace [] = {500, 1000};
		int dcs[] = {10, 20, 30};
		
		
		for(int ss:searchSpace) {
			for(int dc:dcs) {
				String str = "X,S,F,SM,FM,B";
				for(int x:X) {
						init();
						RunMain.create(x, ss, dc, DatasetConfig.schema);
						...
						str +="\n"+x+","+ts+","+tf+","+t_+","+tm+","+block;
						System.out.println("search.space:"+ss+", dc:"+dc+", x:"+x);
						end();
				}
				
				String path = "ploting_"+ss+"x"+ss+"_dc_"+dc+".csv";
...
private static Resource execute_semi(int k) {
	...
	try {
		...
		for(int e=0;e<step;e++) {
			Utils.restartServer();
			MostConsistentFirstSingleOccurence settop = MostConsistentFirstSingleOccurence.loadConfigure();
						
			settop.setK(k);
			settop.algorithm();
			
			if (bb) {
				mem = settop.getRealMemory()/1024;
				bb = false;
			}
...
static void add_over_plan(String str) {
	try {
		String plan = "";
		plan += "=============================================================================";
		plan +="\n";
		plan +=str+"\n\n";
		FileWriter file = new FileWriter("plans", true); 
...
public static Resource memory_and_time(String query) throws Exception {
	...
	String file = DatasetConfig.path+"/file.sql";//"/home/ousissa/eclipse-workspace/TOP_SET_RANK_JOIN/file.sql";
	PrintWriter pw = new PrintWriter(new File(file));
