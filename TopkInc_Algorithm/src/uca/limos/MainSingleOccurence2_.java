...
import uca.limos.api.topk.MostInconsistentFirstSingleOccurence;
//import uca.limos.api.topk.SetTopKAlgorithmV9;

public class MainSingleOccurence2_ {
...
public static void run() throws Exception{
	...
	if (DatasetConfig.fixedk) {
		...
		for(float i = 0.001f; i<=0.2f;i*=5f) {
				init3();
				nextAddingData3(i);
...
public static void nextAddingData(float d) {
	try {
		System.out.println("Start of adding ...");
...
private static Resource execute_semi(int k) {
	...
	try {
		...
		for(int e=0;e<step;e++) {
			MostInconsistentFirstSingleOccurence settop = MostInconsistentFirstSingleOccurence.loadConfigure();
			settop.setK(k);
			settop.algorithm();
			res.addTime(settop.getRealTime());
			if (bb) {
//				mem = settop.getSizeInKo();
				bb = false;
				settop.save("statistiques.csv");
			}
