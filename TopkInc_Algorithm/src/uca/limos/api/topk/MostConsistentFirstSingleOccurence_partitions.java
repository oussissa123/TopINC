...
public class MostConsistentFirstSingleOccurence_partitions extends Results{
	...
	public MostConsistentFirstSingleOccurence_partitions() {
	...
	public static MostConsistentFirstSingleOccurence_partitions loadConfigure() {
		MostConsistentFirstSingleOccurence_partitions loadConfigure = new MostConsistentFirstSingleOccurence_partitions();
			try {
	...
	public static void loadLattices(MostConsistentFirstSingleOccurence_partitions conf) throws SQLException {
		long dc  = 0;
...
//		String inc = "";
		...
		for (int i = 0; i<list.size(); i++) {
			String rel = relations_names.get(i).split("_")[0]+"_"+list.get(i);
			where = where.replace(relations_names.get(i)+".", "t"+i+".");
			from += ", "+rel+" t"+i;			
		}
		//String query = "SELECT * FROM "+from.substring(2) + " WHERE "+ where + " LIMIT "+remaind;
		
		String query = "SELECT * FROM "+from.substring(2) + " WHERE "+ where ;
