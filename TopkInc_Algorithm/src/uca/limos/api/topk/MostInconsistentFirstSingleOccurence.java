...
public class MostInconsistentFirstSingleOccurence extends Results{
...
//	}
	...
	public MostInconsistentFirstSingleOccurence() {
		super();
...
//	}
	...
	public static MostInconsistentFirstSingleOccurence loadConfigure() {
			MostInconsistentFirstSingleOccurence loadConfigure = new MostInconsistentFirstSingleOccurence();
			try {
	...
	public static void loadLattices(MostInconsistentFirstSingleOccurence conf) throws SQLException {
		long dc  = 0;
			...
			if (Long.bitCount(l) <= nbrViol) {
				...
				if (current.isEmpty()) {
					if (Long.bitCount(c) == nbrViol) { 
						...
						restResult = resu - restResult;
						String key = (ll).toString();
						if (!statistiques.containsKey(key))
							statistiques.put(key, 0);
						statistiques.put(key, statistiques.get(key)+restResult);
